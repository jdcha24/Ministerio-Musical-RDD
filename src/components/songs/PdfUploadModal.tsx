...
export const PdfUploadModal: React.FC<Props> = ({ song, isOpen, onClose, onUploaded }) => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'Error al subir el archivo PDF a Firebase Storage');
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl p-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div>
            <h3 className="text-base font-bold text-slate-100">Adjuntar Cifrado / Partitura PDF</h3>
        ...
        <form onSubmit={handleUpload} className="mt-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5">Etiqueta / Tipo</label>
              <input
                type="text"
                value={label}
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              {uploading ? 'Subiendo a Storage...' : 'Subir Cifrado'}
            </button>
          </div>
        </form>
      </div>
