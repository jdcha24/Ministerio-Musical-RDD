...
import { UploadCloud, X, FileText, CheckCircle, HardDrive, Loader2 } from 'lucide-react';
...
export const PdfUploadModal: React.FC<Props> = ({ song, isOpen, onClose, onUploaded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await SongsService.uploadFileToDrive(
        song.id,
        file,
        selectedKey,
      ...
      console.error(err);
      setError(err.message || 'Error al procesar la subida a Google Drive');
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg shadow-2xl p-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-100">Adjuntar Archivo a Google Drive</h3>
              <p className="text-xs text-slate-400">{song.title} - {song.artist}</p>
            </div>
          </div>
        ...
        {error && (
          <div className="my-4 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs leading-relaxed">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5">Nombre / Etiqueta</label>
              <input
                type="text"
                value={label}
          ...
          >
            ...
            {file ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-xs text-slate-400 mt-0.5">{(file.size / 1024).toFixed(1)} KB • Toca para cambiar archivo</p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm font-medium text-slate-200">
                  Selecciona el archivo PDF de tu dispositivo
                </p>
                <p className="text-[11px] text-slate-500 mt-1">El archivo se guardará directamente en tu Google Drive</p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
            ...
            <button
              ...
              className="px-4 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 rounded-xl transition-all shadow-md shadow-indigo-600/20 flex items-center gap-2"
            >
              {uploading ? (
                <>
                ...
                <>
                  ...
                  <span>Subir y Guardar en Drive</span>
                </>
              )}
            </button>
          </div>
